...
async def run_bot_service():
    ...
    application.add_handler(get_forward_handler())  # Обработчик команды /fwd
    application.add_handler(get_help_handler())      # Обработчик команды /help

    # Запускаем бота в режиме вебхука или опроса
    if BOT_URL:
        ...
        log.info("Запуск в режиме опроса")
...
async def set_webhook(application, webhook_uri):
    ...
    log.info('Установка вебхука по URL: %s', webhook_url)

    try:
        if CERTIFICATE_PATH:
            with open(CERTIFICATE_PATH, 'rb') as certificate:
                await application.bot.setWebhook(webhook_url, certificate=certificate)
        else:
